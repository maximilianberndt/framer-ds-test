...
var p = ({ label: e, variant: n, ...r }) => /* @__PURE__ */ t("button", {
	...r,
	className: f("inline-flex items-center px-24 py-12 text-[1.4rem] font-medium leading-none", n === "Secondary" ? "border border-black text-black bg-transparent" : "bg-black text-white", r.className),
	children: e
}), m = (e) => {
...
A();
var I = {};
function L({ loading: e, children: r, name: i, initialBones: o, color: c, darkColor: d, animate: f, stagger: p = !1, transition: m = !1, boneClass: h, className: g, fallback: _, fixture: v, snapshotConfig: y }) {
	...
	let k = c ?? I.color ?? N.web.light, A = d ?? I.darkColor ?? N.web.dark, P = D ? A : k, L = f ?? I.animate ?? "pulse", R = L === !0 ? "pulse" : L === !1 ? "solid" : L;
	a(() => {
		let e = b.current;
		if (!e) return;
	...
	let H = o ?? (i ? ee(i) : void 0), U = B && typeof window < "u" ? window.innerWidth : 0, W = S > 0 ? S : U, G = H && W > 0 ? ne(H, W) : null, K = h ?? I.boneClass, q = (() => {
		let e = p ?? I.stagger;
		return e === !0 ? 80 : e === !1 || !e ? 0 : e;
	})(), J = (() => {
		let e = m ?? I.transition;
			...
			children: n("div", {
				...
				children: [
					G.bones.filter((e) => !T(e).c).map((e, n) => {
						let r = T(e), i = P;
						F(P, D ? M.darkAdjust : M.lightAdjust);
						...
						if (R === "pulse") s.animation = `bp-${x} ${c ?? M.speed} ease-in-out infinite`;
						else if (R === "shimmer") {
							let e = D ? I.darkShimmerColor ?? j.darkHighlight : I.shimmerColor ?? j.lightHighlight, t = I.shimmerAngle ?? j.angle;
							delete s.backgroundColor, s.backgroundImage = `linear-gradient(${t}deg, ${i} ${j.start}%, ${e} 50%, ${i} ${j.end}%)`, s.backgroundSize = "200% 100%", s.animation = `bs-${x} ${c ?? j.speed} linear infinite`;
						}
					...
					R === "pulse" && t("style", { children: `@keyframes bp-${x}{0%,100%{background-color:${P}}50%{background-color:${F(P, D ? M.darkAdjust : M.lightAdjust)}}}` }),
...
function R(e, t, n) {
	let r = t ?? N.runtime, i = n !== !1, a = F(r, M.lightAdjust), o = `${i ? `<style>.boneyard-bone{animation:boneyard-pulse ${M.speed} ease-in-out infinite}@keyframes boneyard-pulse{0%,100%{background-color:${r}}50%{background-color:${a}}}</style>` : ""}<div class="boneyard" style="position:relative;width:100%;height:${e.height}px">`;
...
var G = 1280;
function K({ name: e, show: r, children: i, fixture: a, className: o = "relative w-full", color: s = "#f0f0f0" }) {
	let { width: l } = W(), u = l ?? G, { bonesHtml: d, descriptor: f } = c(() => {
		if (!r) return {};
		let t = B(e);
		if (!t) return {};
		let n = V(t, u);
...
var Y = ({ heading: e = "Projects" }) => /* @__PURE__ */ t(K, {
	name: "project-list",
	show: !1,
	fixture: /* @__PURE__ */ t(J, {
		heading: e,
		items: q
	}),
...
//#endregion
export { b as Badge, p as Button, x as Media, C as PageHero, S as ProjectCard, Y as ProjectList };
